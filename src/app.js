...
const blankReceipt = () => ({ customerName: '', contactNumber: '', petType: 'dog', dogCount: 1, catCount: 1, dogNames: [''], catNames: [''], checkInDate: today(), pickupDate: '', discountReason: '', discountRatePerNight: '', additionalCharges: [{ name: '', amount: '' }], requestId: crypto.randomUUID() });
...
function standardNightlyRate() {
  const nights = nightsBetween(receipt.checkInDate, receipt.pickupDate);
  return ['dog','cat'].reduce((total, type) => {
    if (receipt.petType !== 'mixed' && receipt.petType !== type) return total;
    const count = receipt[`${type}Count`];
    return total + count * rateFor(type, count, nights);
  }, 0);
}
function discountFields() {
  let standard = '';
  try { standard = `${money(standardNightlyRate())} / night`; } catch { standard = 'Choose stay dates'; }
  const reasons = ['Old Customer','Long Term','Many pets','Other'];
  return `<div class="discount-panel"><div class="discount-heading"><div><h3>Discount <span class="optional">(optional)</span></h3><p>Standard booking rate</p></div><strong>${standard}</strong></div><div class="discount-grid"><label class="field"><span>Discount reason</span><select name="discountReason"><option value="">No discount</option>${reasons.map(reason => `<option value="${reason}" ${receipt.discountReason === reason ? 'selected' : ''}>${reason}</option>`).join('')}</select></label>${field('Discounted booking rate / night (Rs.)','discountRatePerNight',receipt.discountRatePerNight,`type="number" min="0.01" max="999999999" step="0.01" inputmode="decimal" placeholder="0.00" ${receipt.discountReason ? 'required' : 'disabled'}`)}</div><p class="discount-note">Enter the total nightly price for all pets. The reason is kept for internal records only.</p></div>`;
}
...
function estimate() {
  try {
    const nights = nightsBetween(receipt.checkInDate, receipt.pickupDate);
    let standardTotal = 0, standardPerNight = 0;
    const lines = ['dog','cat'].flatMap(type => {
      if (receipt.petType !== 'mixed' && receipt.petType !== type) return [];
      const count = receipt[`${type}Count`], rate = rateFor(type, count, nights), nightly = count * rate; standardPerNight += nightly; standardTotal += nightly * nights;
      return `<div class="summary-row"><span>${count} ${type}${count > 1 ? 's' : ''} × ${nights} nights</span><span>${money(nightly)} / night</span></div>`;
    }).join('');
    const enteredDiscount = Number(receipt.discountRatePerNight);
    const validDiscount = receipt.discountReason && Number.isFinite(enteredDiscount) && enteredDiscount > 0 && enteredDiscount < standardPerNight;
    const boardingTotal = validDiscount ? Math.round(enteredDiscount * nights * 100) / 100 : standardTotal;
    ...
    const discount = receipt.discountReason ? `<div class="summary-row standard-price"><span>Standard rate</span><span>${money(standardPerNight)} / night</span></div><div class="summary-row discount-price"><span>Discounted rate</span><strong>${validDiscount ? `${money(enteredDiscount)} / night` : 'Enter rate above'}</strong></div>` : '';
    return `<h2>Stay Summary <span class="badge">${nights} nights</span></h2>${lines}${discount}${chargesTotal ? `<div class="summary-row"><span>Additional charges</span><strong>${money(chargesTotal)}</strong></div>` : ''}<div class="summary-total"><span>Final Total</span><strong>${money(boardingTotal + chargesTotal)}</strong></div>`;
...
function bind() {
  ...
  rf?.elements.discountReason?.addEventListener('change', event => { if (!event.target.value) receipt.discountRatePerNight = ''; render(); });
