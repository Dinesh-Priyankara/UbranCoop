...
test('discounted booking rate replaces the standard nightly total', () => { const input={customerName:'A',contactNumber:'',pets:[{type:'dog',name:'Milo'},{type:'dog',name:'Joe'}],checkInDate:'2026-09-01',pickupDate:'2026-09-03',discountReason:'Long Term',discountRatePerNight:'4500',additionalCharges:[{name:'Transport',amount:'500'}]};const r=calculateReceipt(input);assert.equal(r.standardRatePerNight,5000);assert.equal(r.standardTotal,10000);assert.equal(r.discountRatePerNight,4500);assert.equal(r.discountAmount,1000);assert.equal(r.boardingTotal,9000);assert.equal(r.finalTotal,9500);assert.equal(r.pricingVersion,'2026-09-v3');assert.throws(()=>calculateReceipt({...input,discountReason:'Invalid'}));assert.throws(()=>calculateReceipt({...input,discountReason:'',discountRatePerNight:'4500'}));assert.throws(()=>calculateReceipt({...input,discountRatePerNight:'5000'})); });
